...
- **README 全面更新至 v2.7.4 状态**：品牌与功能全景同步「题屿」（刷题配置入口页 / 顺序随机双槽进度 / 题目搜索 / 内置阅读字体 / 壁纸系统 / 打赏支持 / 模考回看删除）；新增四节交接重点——「刷题会话双槽机制」「打赏弹窗机制」「模考成绩重建与删除限额」「长列表性能规约」，沉淀双槽落盘规约与历史踩坑；DataStore 键全表补至 22 键；源码结构树、技术栈版本、已知限制同步现状；版本历史表补全 v2.3.0 → v2.7.4 共 17 个版本；新增「给接手人的建议」阅读路径
- **`scripts/static_check.py` 入库**：硬编码路径参数化（默认仓库相对定位，支持传参覆盖）；去 ripgrep 硬依赖（缺失自动降级 os.walk，`DQ_SC_NO_RG=1` 可自测）；防误删 must 断言补入 v2.7.4 三项新机制（practiceSessionRandom 双槽 / examDeleteQuota 周限额 / addUsageMs 时长累计）
